...
class TicketView(nextcord.ui.View):
    ...
    @nextcord.ui.button(
        label='Kore help', style=nextcord.ButtonStyle.blurple, custom_id='ticket-view:kore'
    )
    async def kore(self, button: nextcord.ui.Button, interaction: Interaction):
    
        for e in interaction.channel.threads:
            if e.name == f'help - {interaction.user}'.replace('#', ''):
            ...
            await interaction.response.send_message('help created', ephemeral=True)

    @nextcord.ui.button(
        label='Python help', style=nextcord.ButtonStyle.success, custom_id='ticket-view:python'
    )
    async def python(self, button: nextcord.ui.Button, interaction: Interaction):
        await interaction.response.send_message('python', ephemeral=True)
